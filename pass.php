   <?php 
	...
	if (isset($_POST['crepass'])) 
	{
			   ...
			   /* $usr = stripcslashes($usr);
			    ...
			    echo 'variables set';
			    $sql = "SELECT Email FROM member WHERE Email = '$usr'";
                $result = mysqli_fetch_array(mysqli_query($conn,$sql));
			    if($result[0]==$usr){
                   $sql2 = "INSERT INTO memlog (username,password) VALUES ('$usr','$pwd')";

                   mysqli_query($conn,$sql2);
                   header('Location: memberlogin.php?register=yes');
                }
			    else{
			    	header('Location: createpassw.php?register=no');	
			    }
